...
#### Описание решения
Алгоритм `include` выполянется N раз. Выполянется для каждого вектора из набора. Подсчитыватся входит ли вектор `V0` в `VI`
...
#### Описание решения
Пробегаеся циклом `for` по элементам `V` и подсчитываем их в `M`