...
#### Описание решения
Делал используя структуру `vecotor` для реализиции пользовательского ввода маасива. Инициализурем перменную типа `vector<int>` в которую добавляем индексы элеметна . Циклом `for` по длине массива прорбегаемся по немуи добавляем индексы в новый маасив.
Потом возвращаем указатель на массив